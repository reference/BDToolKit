...
@interface BDQuickSecurityCodeUnderline : UIView
...
@implementation BDQuickSecurityCodeUnderline
...
@interface BDQuickSecurityCode()<BDQuickSecurityCodeTextFieldDelegate>
...
@property (nonatomic, strong) NSMutableDictionary<id, BDQuickSecurityCodeUnderline*> *underlines;
@end

@implementation BDQuickSecurityCode
...
-(void) initVariables
{
    ...
    _underlines = [NSMutableDictionary<id, BDQuickSecurityCodeUnderline*> dictionary];
}
...
-(void) setupInit
{
    ...
    for (NSInteger i = 0; i < self.digitsCount; i++)
    {
        ...
        if(self.showUnderlineInsteadOfBorder)
        {
            BDQuickSecurityCodeUnderline *underline = [[BDQuickSecurityCodeUnderline alloc] initWithFrame:CGRectMake(margin+(itemWidth+spacing)*i, ((CGRectGetHeight(self.frame) + itemHeight)/2.0f) - 1.0f, itemWidth, 1.0f)];
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    for (NSInteger i = 0; i < self.digitsCount; i++)
    {
        ...
        if(label.text.length == 0)
        {
            ...
            {
                BDQuickSecurityCodeUnderline *underline = [self.underlines objectForKey:@(i)];
...
- (void)textFieldDidDeleteBackward:(UITextField *)textField {
    for (NSInteger i = self.digitsCount - 1; i >= 0; i--)
    {
        ...
        if(label.text.length != 0)
        {
            ...
            {
                BDQuickSecurityCodeUnderline *underline = [self.underlines objectForKey:@(i+1)];
...
- (void)resetDefaultStatus {
    ...
    {
        __weak typeof(self) weakSelf = self;
        [self.underlines enumerateKeysAndObjectsUsingBlock:^(id  _Nonnull key, BDQuickSecurityCodeUnderline * _Nonnull obj, BOOL * _Nonnull stop) {
